...
| Playlists | `/playlist/` | Video courses, one folder per playlist, each with its own tracker. The hub is a searchable, tag-filtered card grid driven by `playlist/playlists.json`; a playlist page reads its own `data.json` and keeps watch progress, ratings, tags, notes and per-video annotations in the browser. Currently 4 playlists (139 videos): Gemini Enterprise (49, English), Math for Data Science (10, English) and two Arabic courses (47 and 33). |
