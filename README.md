...
│   ├── index.html      # Articles landing page (50 articles grid)
│   ├── Post_Images/    # Hero images for articles (Image_01 through Image_50, .png/.jpg; Fig_NN_* for in-article raster figures)
...
│       └── # 50 article pages, the-paradigm-shift.html … the-benchmark-you-build.html
...
| Technical Articles | `/articles/` | 50-article series on agentic AI, cognitive systems, AI security, and advanced engineering practices (dark-blue theme). Data-carrying diagrams are authored inline SVG (see `the-experience-bottleneck.html`) |
...
| Playlists | `/playlist/` | Video courses, one folder per playlist, each with its own tracker. The hub is a searchable, tag-filtered card grid driven by `playlist/playlists.json`; a playlist page reads its own `data.json` and keeps watch progress, ratings, tags, notes and per-video annotations in the browser. Currently 2 playlists (82 videos): Gemini Enterprise (49, English) and a 33-video Arabic course. |
