...
│   ├── index.html      # Articles landing page (62 articles grid)
...
│       └── # 62 article pages, the-paradigm-shift.html … routing-above-the-model.html (plus verbs-not-nouns.ar.html, the first Arabic RTL edition)
...
| Technical Articles | `/articles/` | 62-article series on agentic AI, cognitive systems, AI security, and advanced engineering practices (dark-blue theme). Data-carrying diagrams are authored inline SVG (see `the-experience-bottleneck.html`); heroes are authored as vector art and exported to `Post_Images/Image_NN.png` |
...
| Habit Tracker | `/apps/habit-tracker/` | Full-screen day board of the same nine micro-habits: Attention, Energy and Happiness stand as three framed columns of three, one tap marks a habit done for today and a ring lights the box, the box fills bottom-up with how many of the last seven days are done, a flame counts the run, hovering a box enlarges it, and every habit carries its own hand-drawn symbol |
...
| Life Track | `/apps/life-track/` | Full-screen board of four groups of seven — Life (Allah, Family, Profession, Independence, Social, Health, Finance), seven Suras, seven bodyweight exercises, seven adhkar — Life and the Suras take one tap while the exercises and adhkar fill over seven, every box carries its own symbol and doubles on hover, and the board resets at midnight |
| Zikr Tracker | `/apps/zikr-tracker/` | Full-screen board of the daily adhkar in five panels — Morning, Evening, After Salah, Before Sleep, and Safar for the road — one tap marks a dhikr done for today and a ring lights the box, the box fills bottom-up with how many of the last seven days are done, a seven-bar strip shows which days and a flame counts the run, hovering a box replaces its symbol with the du'a in Arabic, and the Safar panel is dashed and deliberately unscored so travel azkar never break a streak or hold back a perfect day |
| Ruku Reader | `/apps/ruku-reader/` | Browse the Quran by ruku — a grid of all 114 surahs, a popup listing every ruku in the surah you pick with its ayah range and status, and a full-page reader that sets the ruku in flowing Uthmani script with a bismillah where one belongs, seamless prev/next paging across surah boundaries over all 551 rukus, hide-to-recall modes for memorization that either blur every ayah or remove it outright, each opening in two clicks — the first few words as a cue, then the whole verse, then hidden again for another rep (h cycles, r reveals all), a small-type hook at the foot of the page carrying the opening words of the next ruku so the seam between rukus gets rehearsed too (n expands it to the full ayah), four switchable Arabic typefaces including Amiri Quran for Uthmani rendering (f cycles), a recitation player docked at the foot of the reader that plays the whole ruku end to end and repeats the whole thing 1, 2, 3, 5, 10 times or on an endless loop — nine reciters, 0.75×–1.5× speed, the recited ayah highlighted and scrolled to, and a Continue toggle that rolls on into the next ruku so you can recite straight through a run of them (space plays and pauses), adjustable Arabic text size, dark and light reading modes, and New / Reading / Done tracking that fills a progress rail on every surah card |
