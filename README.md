...
│   ├── index.html      # Articles landing page (53 articles grid)
...
│       └── # 53 article pages, the-paradigm-shift.html … promise-eighty-deliver-hundred.html
...
| Playlists | `/playlist/` | Video courses, one folder per playlist, each with its own tracker. The hub is a searchable, tag-filtered card grid driven by `playlist/playlists.json`; a playlist page reads its own `data.json` and keeps watch progress, ratings, tags, notes and per-video annotations in the browser. Currently 5 playlists (174 videos): Gemini Enterprise (49, English), Math for Data Science (10, English), The Balance System (35, English) and two Arabic courses (47 and 33). |
